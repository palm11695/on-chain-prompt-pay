...
test(
  ...
  async () => {
    ...
    // const rawEmail: Buffer = fs.readFileSync(path.join(__dirname, '../emls/zktestemail_twitter.eml'), 'utf8')
    const rawEmail = fs.readFileSync(path.join(__dirname, '../emls/kbank.eml'), 'utf8')
    dkimResult = await verifyDKIMSignature(rawEmail)
    // dkimResult = await verifyDKIMSignature(Buffer.from(rawEmail.toString().replace('DKIM-Signature', 'dkim-signature')))
    ...
    // console.log(util.inspect(emailVerifierInputs.in_padded, { maxArrayLength: null }))
    // console.log(util.inspect(emailVerifierInputs.in_body_padded, { maxArrayLength: null }))

    const enc = new TextEncoder()
    const dec = new TextDecoder('utf-8')
    // emailVerifierInputs.in_padded = Uint8ArrayToCharArray(
    //   enc.encode(
    //     dec
    //       .decode(new Uint8Array(emailVerifierInputs.in_padded.map((c) => Number(c))))
    //       .replace('DKIM-Signature', 'dkim-signature'),
    //   ),
    // ).slice(0, 1024)
    console.log('len in_padded', emailVerifierInputs.in_padded?.length)
    console.log('string in_padded', dec.decode(new Uint8Array(emailVerifierInputs.in_padded.map((c) => Number(c)))))
    console.log('len in_body_padded', emailVerifierInputs.in_body_padded?.length)
