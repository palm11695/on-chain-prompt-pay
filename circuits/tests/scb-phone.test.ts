...
beforeAll(async () => {
  ...
  if (!skipCircuit) {
    circuit = await wasm_tester(path.join(__dirname, '../scb_phone.circom'), {
      ...
      recompile: true,
      output: path.join(__dirname, '../build/scb_phone'),
      include: path.join(__dirname, '../node_modules'),
    })
  }

  const rawEmail = fs.readFileSync(path.join(__dirname, '../emls/scb_phone.eml'), 'utf8')
